            ...
            <div class="col-md-9">
                <div class="d-flex justify-content-end">
                    <div class="stats-card me-3" style="min-width: 200px;">
                        <div class="stats-number">{{ number_format($doanhThuTheoPhim->sum(function($phim) { return $phim->sum('tong_doanh_thu'); })) }}đ</div>
                        <div>Tổng doanh thu năm {{ $selectedYear }}</div>
                    </div>
                    <div class="stats-card" style="min-width: 200px;">
                        <div class="stats-number">{{ $doanhThuTheoPhim->sum(function($phim) { return $phim->sum('so_ve_ban'); }) }}</div>
                ...
                <div class="tab-pane fade {{ $month == 1 ? 'show active' : '' }}" 
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-striped table-hover">
                                            ...
                                            <tbody>
                                                @foreach($thangData[$month] as $suatChieu)
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <span class="badge bg-info">{{ $suatChieu->so_ve_ban }} vé</span>
                                                        </td>
                                                        <td>
                                                            <span class="badge bg-success">{{ number_format($suatChieu->doanh_thu_suat) }} đ</span>
                                                        </td>
                                                    </tr>
                                                @endforeach
                                            </tbody>
                                            <tfoot class="table-secondary">
                                                <tr>
                                                    <th colspan="4">Tổng phim {{ $tenPhim }} - Tháng {{ $month }}</th>
                                                    <th>{{ $thangData[$month]->sum('so_ve_ban') }} vé</th>
                                                    <th>{{ number_format($thangData[$month]->sum('doanh_thu_suat')) }} đ</th>
                                                </tr>
                                            </tfoot>
                                        </table>
                                    </div>
                    ...
                    @else
                        <div class="alert alert-info">
                            <i class="fas fa-info-circle"></i> Không có dữ liệu cho tháng {{ $month }}/{{ $selectedYear }}
                        </div>
                    @endif
                </div>
            @endfor
