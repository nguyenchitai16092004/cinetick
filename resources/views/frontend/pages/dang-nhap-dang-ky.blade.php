...
<div class="auth-container">
    <div class="auth-card">
        ...
        <div class="auth-form-wrapper" id="form-wrapper" style="height:auto;">
            <!-- Login Form -->
            <form class="auth-form active" id="login-form" autocomplete="off" action="{{ route('login') }}" method="POST">
                @csrf
                <div class="form-group">
                    <label class="form-label required">Email / Tên đăng nhập</label>
                    <input type="text" class="form-input" name="TenDN" placeholder="Email / Tên đăng nhập" required autocomplete="username" value="{{ old('TenDN') }}">
                    @error('TenDN')
                ...
                <div class="form-group" style="position:relative;">
                    ...
                    <input type="password" class="form-input" id="login-password" name="MatKhau" placeholder="Mật khẩu" required minlength="6" autocomplete="current-password">
            ...
            <!-- Register Form -->
            <form class="auth-form" id="register-form" autocomplete="off" action="{{ route('register.form.post') }}" method="POST">
                @csrf
                <div class="form-title">Đăng ký tài khoản thành viên và nhận ngay ưu đãi!</div>
                <div class="form-group">
                    <label class="form-label required">Số CCCD</label>
                    <input type="text" class="form-input" id="rgCCCD" name="ID_CCCD" placeholder="Số CCCD" value="{{ old('ID_CCCD') }}" required pattern="^\d{12}$" title="Số CCCD chỉ có thể là 12 chữ số">
                    @error('ID_CCCD')
                        <div class="alert">{{ $message }}</div>
                    @enderror
                </div>
                <div class="form-group">
                    ...
                    <input type="text" class="form-input" id="rgFullName" name="HoTen" placeholder="Họ &amp; tên" value="{{ old('HoTen') }}" required>
                    @error('HoTen')
                        <div class="alert">{{ $message }}</div>
                    @enderror
                </div>
                <div class="form-group">
                    ...
                    <input type="email" class="form-input" id="rgEmail" name="Email" placeholder="Email" value="{{ old('Email') }}" required>
                    @error('Email')
                        <div class="alert">{{ $message }}</div>
                    @enderror
                </div>
                <div class="form-group">
                    ...
                    <input type="tel" class="form-input" id="rgPhone" name="SDT" placeholder="Điện thoại" value="{{ old('SDT') }}" required pattern="\d{10,11}" title="Số điện thoại phải là dãy số từ 10 đến 11 chữ số">
                    @error('SDT')
                        <div class="alert">{{ $message }}</div>
                    @enderror
                </div>
                <div class="form-group" style="position:relative;">
                    ...
                    <input type="date" id="rgBirthDay" name="NgaySinh" class="form-input" value="{{ old('NgaySinh') }}" required>
                    @error('NgaySinh')
                        <div class="alert">{{ $message }}</div>
                    @enderror
                </div>
                <div class="form-group">
                    ...
                    <input type="text" class="form-input" id="rgUserName" name="TenDN" placeholder="Tên đăng nhập" value="{{ old('TenDN') }}" required autocomplete="username">
                    @error('TenDN')
                        <div class="alert">{{ $message }}</div>
                    @enderror
                </div>
                <div class="form-group" style="position:relative;">
                    <label class="form-label required">Mật khẩu</label>
                    <input type="password" class="form-input" id="reg-password" name="MatKhau" placeholder="Mật khẩu" required autocomplete="new-password">
                    <button type="button" class="input-icon" id="toggle-reg-pw" tabindex="-1"><i class="fa-regular fa-eye"></i></button>
                    @error('MatKhau')
                        <div class="alert">{{ $message }}</div>
                    @enderror
                </div>
                <div class="form-group" style="position:relative;">
                    ...
                    <input type="password" class="form-input" id="reg-password2" name="MatKhau_confirmation" placeholder="Mật khẩu nhập lại" required>
                    <button type="button" class="input-icon" id="toggle-reg-pw2" tabindex="-1"><i class="fa-regular fa-eye"></i></button>
                ...
                <div class="form-group">
                    ...
                    <div style="display: flex; gap: 24px; align-items: center;">
                        <label style="display: flex; align-items: center; gap: 6px; font-weight: 400;">
                            <input type="radio" name="GioiTinh" value="1" {{ old('GioiTinh', '1') == '1' ? 'checked' : '' }}>
                            Nam
                        </label>
                        <label style="display: flex; align-items: center; gap: 6px; font-weight: 400;">
                            <input type="radio" name="GioiTinh" value="0" {{ old('GioiTinh') == '0' ? 'checked' : '' }}>
                            Nữ
                        </label>
                    </div>
                    @error('GioiTinh')
                ...
                <p style="color: #333; margin-top: 10px;">Vui lòng nhập đầy đủ thông tin vào các trường có đánh dấu <b style="color: red;">(*)</b></p>
            </form>
        </div>
    </div>
</div>
...
(function() {
    ...
    loginTab.addEventListener('keydown', e=>{if(e.key==='Enter') showForm('login');});
    registerTab.addEventListener('keydown', e=>{if(e.key==='Enter') showForm('register');});
    ...
    window.addEventListener('DOMContentLoaded', ()=>{
        ...
        formWrapper.style.height = (active === 'register' ? registerForm.offsetHeight : loginForm.offsetHeight) + "px";
    });
