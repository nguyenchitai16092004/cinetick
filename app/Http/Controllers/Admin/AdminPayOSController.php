...
class AdminPayOSController extends Controller
{
    protected $payOS;
    ...
    public function createPaymentLink(array $orderData)
    {
        ...
        try {
            $response = $this->payOS->createPaymentLink([
                ...
                // Sử dụng route callback riêng cho admin
                'returnUrl'   => route('hoa-don.payos.return') . '?orderCode=' . $orderCodeNum,
                'cancelUrl'   => route('hoa-don.payos.cancel') . '?orderCode=' . $orderCodeNum,
    ...
    // Thêm các hàm callback cho admin nếu chưa có
